        ...
        try:
            ...
            for dynamic_rule in dynamic_predicates:
                try:
                    ...
                    print(f"Warning: Could not process dynamic predicate {dynamic_rule}: {e}")
    ...
    def parse_statement(self, text: str) -> Optional[Tuple[str, List[str]]]:
        ...
        for rel_type, pattern_list in patterns.items():
            for pattern in pattern_list:
                ...
                if match:
                    names = [name.capitalize() for name in match.groups()]
        ...
        if rel_type in ['mother', 'father', 'child', 'daughter', 'son']:
            ...
            try:
                if rel_type in ['mother', 'father']:
                    ...
                    result = list(self.prolog.query(f"ancestor({person2}, {person1})"))[:5]
                    if result:
                        return False
                elif rel_type in ['child', 'daughter', 'son']:
                    # Check if parent is already a child of the child
                    result = list(self.prolog.query(f"ancestor({person1}, {person2})"))[:5]
                    if result:
                        return False
            except:
                pass
    ...
    def add_fact(self, rel_type: str, names: List[str]) -> str:
        ...
        try:
            if rel_type == 'mother':
                ...
                return f"OK! I learned something."
                ...
                self.prolog.assertz(f"female({aunt})")
                return f"OK! I learned something."
    ...
    def query_relationship(self, query_type: str, names: List[str]) -> str:
        ...
        try:
            if query_type == 'are_siblings':
                ...
                grandmother, grandchild = names
                result = list(self.prolog.query(f"grandmother({grandmother}, {grandchild})"))
                return "Yes!" if result else "No!"
            
            elif query_type == 'is_grandfather':
                grandfather, grandchild = names
                result = list(self.prolog.query(f"grandfather({grandfather}, {grandchild})"))
                return "Yes!" if result else "No!"
                ...
                person = names[0]
                daughters = list(self.prolog.query(f"daughter(Daughter, {person})"))[:10]
                if daughters:
    ...
    def process_input(self, user_input: str) -> str:
        ...
        if user_input.strip().endswith('?'):
            ...
            if parsed:
                ...
                return self.query_relationship(query_type, names)
            else:
        ...
        # Check if it's a statement
            ...
            if parsed:
                ...
                if self.check_feasibility(rel_type, names):
